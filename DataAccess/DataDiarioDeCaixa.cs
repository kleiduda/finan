...
namespace DataAccess
{
    public class DataDiarioDeCaixa : Connection
    {
        ...
        public DataTable DiarioDeCaixaResultado()
        {
            using (var connection = GetConnection())
            {
                ...
                try
                {
                    ...
                    command.CommandText = "select te.descricao, te.dinheiro, SUM(valor) as Valor, CONVERT(varchar(10), e.data_entrada, 103) as Data " +
                        "from tb_entrada e INNER JOIN tb_tipo_entrada te ON e.id_tipo_entrada = te.id GROUP BY te.descricao, e.data_entrada, te.dinheiro " +
            ...
            using (var connection = GetConnection())
            {
                ...
                try
                {
                    ...
                    command.CommandText = "select ts.descricao, ts.dinheiro, SUM(valor) as Valor, CONVERT(varchar(10), s.data_saida, 103) as Data " +
                        "from tb_saida s INNER JOIN tb_tipo_saida ts ON s.id_tipo_saida = ts.id GROUP BY ts.descricao, s.data_saida, ts.dinheiro " +
